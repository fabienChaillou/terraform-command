...
//  2. Validate the request arguments (via terraform.Validator).
//  3. Build the flat CLI argument slice (via terraform.ArgBuilder).
...
// *terraform.Registry[terraform.Command] satisfies this interface via
// structural typing.
...
type CommandOutput struct {
	Body *terraform.ExecutionResult
}
...
type Config struct {
	...
	MaxAttemptsByAction terraform.ActionMap[int32]
}
...
func isSubcmdError(errs []terraform.ValidationError) bool {
	for _, e := range errs {
