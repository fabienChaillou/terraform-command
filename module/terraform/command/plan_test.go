...
func TestPlanValidate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:       &terraform.Request{Action: "plan"},
			wantErrs:  1,
			errFields: []string{"args"},
		},
		{
			name:     "var map is valid",
			req:      &terraform.Request{Action: "plan", Args: map[string]interface{}{"var": map[string]interface{}{"env": "prod"}}},
			wantErrs: 0,
		},
		{
			name:     "var-file string is valid",
			req:      &terraform.Request{Action: "plan", Args: map[string]interface{}{"var-file": "vars.tfvars"}},
			wantErrs: 0,
		},
		{
			name:     "target string is valid",
			req:      &terraform.Request{Action: "plan", Args: map[string]interface{}{"target": "aws_vpc.main"}},
			wantErrs: 0,
		},
		{
			name:     "out string is valid",
			req:      &terraform.Request{Action: "plan", Args: map[string]interface{}{"out": "plan.tfplan"}},
			wantErrs: 0,
		},
		{
			name:     "destroy bool is valid",
			req:      &terraform.Request{Action: "plan", Args: map[string]interface{}{"destroy": true}},
			wantErrs: 0,
		},
		{
			name:     "no-color bool is valid",
			req:      &terraform.Request{Action: "plan", Args: map[string]interface{}{"no-color": true}},
			wantErrs: 0,
		},
		{
			name:      "var as string is invalid (must be map)",
			req:       &terraform.Request{Action: "plan", Args: map[string]interface{}{"var": "env=prod"}},
			wantErrs:  1,
			errFields: []string{"var"},
		},
		{
			name:      "var as bool is invalid",
			req:       &terraform.Request{Action: "plan", Args: map[string]interface{}{"var": true}},
			wantErrs:  1,
			errFields: []string{"var"},
		},
		{
			name:      "var-file empty string is invalid",
			req:       &terraform.Request{Action: "plan", Args: map[string]interface{}{"var-file": ""}},
			wantErrs:  1,
		...
		},
		{
			...
			req:       &terraform.Request{Action: "plan", Args: map[string]interface{}{"var-file": 42}},
			wantErrs:  1,
			errFields: []string{"var-file"},
		},
		{
			name:      "unknown flag produces error",
			req:       &terraform.Request{Action: "plan", Args: map[string]interface{}{"refresh": true}},
			wantErrs:  1,
			errFields: []string{"refresh"},
		},
		{
			name:      "multiple validation errors",
			req:       &terraform.Request{Action: "plan", Args: map[string]interface{}{"var": "bad", "bad-flag": true}},
			wantErrs:  2,
...
func TestPlanBuildArgs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			req:  &terraform.Request{Args: map[string]interface{}{"var": map[string]interface{}{"env": "prod"}}},
			// -var must be followed immediately by the key=value pair
			wantArgs: []string{"-var"},
		},
		{
			name:     "var-file flag",
			req:      &terraform.Request{Args: map[string]interface{}{"var-file": "vars.tfvars"}},
		...
		{
			name:     "out flag",
			req:      &terraform.Request{Args: map[string]interface{}{"out": "plan.out"}},
			wantArgs: []string{"-out=plan.out"},
		},
		{
			name:     "destroy flag",
			req:      &terraform.Request{Args: map[string]interface{}{"destroy": true}},
		...
		},
		{
			...
			req:        &terraform.Request{Args: map[string]interface{}{"destroy": false, "no-color": false}},
...
func TestPlanBuildArgs_VarKeyValue(t *testing.T) {
	cmd := &PlanCommand{}
	req := &terraform.Request{Args: map[string]interface{}{
...
// assertErrors is a shared helper used by plan, apply, destroy, workspace tests.
func assertErrors(t *testing.T, errs []terraform.ValidationError, wantCount int, wantFields []string) {
	t.Helper()
	if wantCount == 0 {
		if len(errs) != 0 {
