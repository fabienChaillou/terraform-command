...
func TestApplyValidate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:       &Request{Action: "apply"},
			wantErrs:  1,
			errFields: []string{"args"},
		},
		{
			name:     "auto-approve bool is valid",
			req:      &Request{Action: "apply", Args: map[string]interface{}{"auto-approve": true}},
			wantErrs: 0,
		},
		{
			name:     "var map is valid",
			req:      &Request{Action: "apply", Args: map[string]interface{}{"var": map[string]interface{}{"env": "staging"}}},
			wantErrs: 0,
		},
		{
			name:     "var-file string is valid",
			req:      &Request{Action: "apply", Args: map[string]interface{}{"var-file": "staging.tfvars"}},
			wantErrs: 0,
		},
		{
			name:     "target string is valid",
			req:      &Request{Action: "apply", Args: map[string]interface{}{"target": "module.vpc"}},
			wantErrs: 0,
		},
		{
			name:     "no-color bool is valid",
			req:      &Request{Action: "apply", Args: map[string]interface{}{"no-color": true}},
			wantErrs: 0,
		},
		{
			name: "all valid flags combined",
			req: &Request{Action: "apply", Args: map[string]interface{}{
				"auto-approve": true,
		...
		},
		{
			...
			req:       &Request{Action: "apply", Args: map[string]interface{}{"var": "env=prod"}},
			wantErrs:  1,
			errFields: []string{"var"},
		},
		{
			name:      "var-file empty string is invalid",
			req:       &Request{Action: "apply", Args: map[string]interface{}{"var-file": ""}},
			wantErrs:  1,
			errFields: []string{"var-file"},
		},
		{
			name:      "unknown flag -parallelism is invalid",
			req:       &Request{Action: "apply", Args: map[string]interface{}{"parallelism": 10}},
			wantErrs:  1,
			errFields: []string{"parallelism"},
		},
		{
			name:      "multiple errors: bad var and unknown flag",
			req:       &Request{Action: "apply", Args: map[string]interface{}{"var": 42, "unknown": true}},
			wantErrs:  2,
...
func TestApplyBuildArgs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:      &Request{Args: map[string]interface{}{"auto-approve": true}},
		...
		{
			name:     "var-file flag",
			req:      &Request{Args: map[string]interface{}{"var-file": "prod.tfvars"}},
			wantArgs: []string{"-var-file=prod.tfvars"},
		},
		{
			name:     "target flag",
			req:      &Request{Args: map[string]interface{}{"target": "aws_instance.web"}},
		...
		},
		{
			...
			req:        &Request{Args: map[string]interface{}{"auto-approve": false}},
			wantAbsent: []string{"-auto-approve"},
		},
		{
			name:     "var map",
			req:      &Request{Args: map[string]interface{}{"var": map[string]interface{}{"region": "eu-west-1"}}},
			wantArgs: []string{"-var"},
		},
	}
