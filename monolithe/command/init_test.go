...
func TestInitValidate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:       &Request{Action: "init"},
			wantErrs:  1,
			errFields: []string{"args"},
		},
		{
			name:     "backend-config is valid",
			req:      &Request{Action: "init", Args: map[string]interface{}{"backend-config": "backend.hcl"}},
			wantErrs: 0,
		},
		{
			name:     "reconfigure bool true is valid",
			req:      &Request{Action: "init", Args: map[string]interface{}{"reconfigure": true}},
			wantErrs: 0,
		},
		{
			name:     "upgrade bool true is valid",
			req:      &Request{Action: "init", Args: map[string]interface{}{"upgrade": true}},
			wantErrs: 0,
		},
		{
			name:     "no-color bool true is valid",
			req:      &Request{Action: "init", Args: map[string]interface{}{"no-color": true}},
			wantErrs: 0,
		},
		{
			name:     "all known flags are valid",
			req:      &Request{Action: "init", Args: map[string]interface{}{"reconfigure": true, "upgrade": true, "no-color": true, "backend-config": "b.hcl"}},
			wantErrs: 0,
		},
		{
			name:      "unknown flag produces error",
			req:       &Request{Action: "init", Args: map[string]interface{}{"unknown-flag": "value"}},
			wantErrs:  1,
			errFields: []string{"unknown-flag"},
		},
		{
			name:     "multiple unknown flags each produce an error",
			req:      &Request{Action: "init", Args: map[string]interface{}{"bad1": true, "bad2": true}},
			wantErrs: 2,
		},
		{
			name:      "mix of valid and unknown flag",
			req:       &Request{Action: "init", Args: map[string]interface{}{"reconfigure": true, "bad-flag": true}},
			wantErrs:  1,
...
func TestInitBuildArgs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:        &Request{Args: map[string]interface{}{"reconfigure": true}},
			firstArgIs: "init",
		},
		{
			name:     "backend-config flag",
			req:      &Request{Args: map[string]interface{}{"backend-config": "backend.hcl"}},
		...
		{
			name:     "upgrade bool",
			req:      &Request{Args: map[string]interface{}{"upgrade": true}},
		...
		},
		{
			...
			req:        &Request{Args: map[string]interface{}{"reconfigure": false}},
			wantAbsent: []string{"-reconfigure"},
		},
		{
			name:     "all flags combined",
			req:      &Request{Args: map[string]interface{}{"backend-config": "b.hcl", "reconfigure": true, "upgrade": true, "no-color": true}},
...
// ── helpers used across all command tests ────────────────────────────────────
