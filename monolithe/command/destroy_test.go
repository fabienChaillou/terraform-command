...
func TestDestroyValidate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:       &Request{Action: "destroy"},
			wantErrs:  1,
			errFields: []string{"args"},
		},
		{
			name:     "auto-approve bool is valid",
			req:      &Request{Action: "destroy", Args: map[string]interface{}{"auto-approve": true}},
			wantErrs: 0,
		},
		{
			name:     "var map is valid",
			req:      &Request{Action: "destroy", Args: map[string]interface{}{"var": map[string]interface{}{"env": "dev"}}},
			wantErrs: 0,
		},
		{
			name:     "var-file string is valid",
			req:      &Request{Action: "destroy", Args: map[string]interface{}{"var-file": "dev.tfvars"}},
			wantErrs: 0,
		},
		{
			name:     "target string is valid",
			req:      &Request{Action: "destroy", Args: map[string]interface{}{"target": "aws_s3_bucket.logs"}},
			wantErrs: 0,
		},
		{
			name:     "no-color bool is valid",
			req:      &Request{Action: "destroy", Args: map[string]interface{}{"no-color": true}},
			wantErrs: 0,
		},
		{
			name: "all valid flags combined",
			req: &Request{Action: "destroy", Args: map[string]interface{}{
				"auto-approve": true,
		...
		},
		{
			...
			req:       &Request{Action: "destroy", Args: map[string]interface{}{"var": []string{"env=dev"}}},
			wantErrs:  1,
			errFields: []string{"var"},
		},
		{
			name:      "var-file empty string is invalid",
			req:       &Request{Action: "destroy", Args: map[string]interface{}{"var-file": ""}},
			wantErrs:  1,
			errFields: []string{"var-file"},
		},
		{
			name:      "unknown flag produces error",
			req:       &Request{Action: "destroy", Args: map[string]interface{}{"state": "terraform.tfstate"}},
			wantErrs:  1,
...
func TestDestroyBuildArgs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "var-file flag",
			req:      &Request{Args: map[string]interface{}{"var-file": "dev.tfvars"}},
		...
		},
		{
			...
			req:        &Request{Args: map[string]interface{}{"auto-approve": false, "no-color": false}},
			wantAbsent: []string{"-auto-approve", "-no-color"},
		},
		{
			name:     "var map expands",
			req:      &Request{Args: map[string]interface{}{"var": map[string]interface{}{"env": "dev"}}},
			wantArgs: []string{"-var"},
		},
	}
