...
// WorkspaceCommand implements Command for "terraform workspace".
...
func (c *WorkspaceCommand) Validate(req *Request) []ValidationError {
...
func (c *WorkspaceCommand) BuildArgs(req *Request) []string {
...
func validateWorkspaceNew(req *Request) []ValidationError {
	...
	if e := requireStringArg(req.Args, "name",
		...
		errs = append(errs, ValidationError{Field: "name", Message: "workspace name must not contain spaces"})
	}

	if v, ok := req.Args["state"]; ok {
		if s, isStr := v.(string); !isStr || strings.TrimSpace(s) == "" {
			errs = append(errs, ValidationError{Field: "state", Message: "-state must be a non-empty file path"})
		}
	}
...
func validateWorkspaceSelect(req *Request) []ValidationError {
	var errs []ValidationError
	if e := requireStringArg(req.Args, "name",
...
func validateWorkspaceDelete(req *Request) []ValidationError {
	var errs []ValidationError
	if e := requireStringArg(req.Args, "name",
...
func validateNoArgs(req *Request, cmdLabel string) []ValidationError {
	var errs []ValidationError
